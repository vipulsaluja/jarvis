...
- [x] T3: Single-turn Claude API round-trip
  `agent/jarvis.py` sends one user message with the system prompt and prints the response. Requires `ANTHROPIC_API_KEY` from env. Running it directly produces a real response from the API.
...
- [x] T5: Add 2-3 hardcoded tools with execution loop
  Tool definitions for `get_current_time` and `open_url` (plus optionally `get_weather_stub`) added to `tools/`. Agent parses tool use blocks from the API response, executes the right function, and sends the tool result back in the next API call. Asking "what time is it?" triggers the tool end-to-end.
